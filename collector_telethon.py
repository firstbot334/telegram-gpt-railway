...
def _require_env():
    missing = [k for k in ["TELEGRAM_API_ID","TELEGRAM_API_HASH","TELETHON_SESSION","DATABASE_URL"] if not os.getenv(k)]
    if missing:
        raise RuntimeError(f"Missing required env(s): {', '.join(missing)}")

def _ensure_sources(sources):
    return sources if isinstance(sources, (list, tuple)) else [sources]
...
async def collect():
    ...
    if not await client.is_user_authorized():
        raise RuntimeError("TELETHON_SESSION not authorized. Recreate locally and set env again.")
